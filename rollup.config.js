  ...
  return {
    writeBundle(bundle) {
      const bundleSize = parseFloat(
        Math.round(fsizeSync(pkg.files[0])) / 1024
      ).toFixed(2);
      const filesPaths = [];
      ...
      for (var i = 0; i < items.length; i++) {
        ...
        if (extension === ".js") {
          filesPaths.push(
            chalk.white.bold("js:     ") + chalk.white(`${items[i]}`)
          );
        } else if (extension === ".css") {
          filesPaths.push(
            chalk.white.bold("css:    ") + chalk.white(`${items[i]}`)
          );
        }
      }
      ...
      if (PRODUCTION) {
        const productionContent = [
          ...
          chalk.white.bold("size:   ") +
            chalk.white(`${bundleSize}KB`) +
            chalk.gray(" (Keep it under 100KB)"),
          chalk.white.bold("guide:  ") +
            chalk.white("https://docs.figmaplus.com/#/developerGuide/publish")
        ];
